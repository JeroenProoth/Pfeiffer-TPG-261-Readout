...
def adaptive_reduce_size(dataframe, error = 0.0005):
    '''Reduces dataframe size by excluding any variations
    below a certain error (%) change.
    '''
    ...
    return pd.DataFrame(
        new_list,
        columns = ['Time (s)', 'Pressure Turbo (mbar)', 'Pressure Evap (mbar)', 'Frequency (Hz)']
        )


def calculate_on_df(dataframe, baseline):
    '''Calculates the frequency change from a baseline frequency.'''
